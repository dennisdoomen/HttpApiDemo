...
public class ApiApproval
{
    ...
    public Task ApproveApi(string framework)
    {
        ...
        var publicApi = assembly.GeneratePublicApi(new ApiGeneratorOptions { IncludeAssemblyAttributes = false });
    ...
    private class TargetFrameworksTheoryData : TheoryData<string>
    {
        public TargetFrameworksTheoryData()
        {
            ...
            var element = project.XPathSelectElement("/Project/PropertyGroup/TargetFrameworks")
                ?? project.XPathSelectElement("/Project/PropertyGroup/TargetFramework");
            AddRange(element!.Value.Split(';'));
        }
    }
}
